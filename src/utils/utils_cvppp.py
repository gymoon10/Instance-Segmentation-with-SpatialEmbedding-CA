...
class Cluster:
    ...
    def cluster(self, prediction, n_sigma=1, threshold=0.5):
        ...
        if mask.sum() > 128:
            ...
            spatial_emb_masked = spatial_emb[mask.expand_as(spatial_emb)].view(2, -1)  # (2, N)
            sigma_masked = sigma[mask.expand_as(sigma)].view(n_sigma, -1)  # (1, N)
            seed_map_masked = seed_map[mask].view(1, -1)  # (1, N)
