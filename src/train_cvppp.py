...
if args['model']['name'] == 'branched-erfnet' or args['model']['name'] == 'branched-erfnet-detail':
...
def train(epoch):
    ...
    for i, sample in enumerate(tqdm(train_dataset_it)):
        ...
        instances = sample['instance'].squeeze(1)  # GT-instance-mask : (N, h, w)
        class_labels = sample['label'].squeeze(1)  # GT-semantic-mask : (N, h, w)
...
def val(epoch):
    ...
    with torch.no_grad():
        ...
        for i, sample in enumerate(tqdm(val_dataset_it)):
            ...
            im = sample['image']  # (N, 3, 512, 512)
            instances = sample['instance'].squeeze(1)  # instance-mask : (N, h, w)
            class_labels = sample['label'].squeeze(1)  # semantic-mask : (N, h, w)
